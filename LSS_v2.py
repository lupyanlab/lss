...
#v2 change - use png instead of jpg; show picture and mask on the other side so there is vis stim on both sides
...
class Exp:
    def __init__(self):
        ...
        self.sideOpposites = {'left':'right', 'right':'left',}
...
In this experiment, you will hear various words like 'dog' and 'guitar' followed by pictures of common objects or animals appearing on the left or right side of the screen. Sometimes the words will match the pictures and sometimes they won't, but it doesn't matter for your responses. All you need to do is respond as quickly as possible whether the meaningful picture is on the left or the right side of the screen. One side will always contain some meaningful picture. The other side will contain some jumbled up colors. You should respond LEFT if the meaningful picture is on the left and RIGHT if it's on the right.
...
class ExpPresentation(Exp):
    ...
    def initializeExperiment(self):
        ...
        self.pictureMatrix = loadFiles('stimuli/pictures', 'png', 'image', self.experiment.win)
    ...
    def presentTestTrial(self, whichPart, curTrial, curTrialIndex):
        ...
        curTarget = self.pictureMatrix[curTrial['pic_file']]['stim']
        curTargetMask = self.pictureMatrix[curTrial['pic_file']+'_mask']['stim']
        curDistractorMask = copy.copy(self.pictureMatrix[curTrial['pic_file']+'_mask']['stim'])

        curTarget.setPos(self.experiment.stimPositions[curTrial['side']])
        curTargetMask.setPos(self.experiment.stimPositions[curTrial['side']])
        curDistractorMask.setPos(self.experiment.stimPositions[self.experiment.sideOpposites[curTrial['side']]])
        print curTarget.pos
        print curTargetMask.pos
        print curDistractorMask.pos
        ...
        setAndPresentStimulus(self.experiment.win,[self.fixSpot,curTargetMask,curTarget,curDistractorMask])
        ...
        [header, curLine] = createRespNew(self.experiment.subjInfo, self.experiment.subjVariables, self.fieldNames, fieldVars,
                                        a_expName = self.experiment.expName,
                                        b_whichPart = whichPart,
                                        c_curTrialIndex = curTrialIndex,
                                        d_expTimer = self.expTimer.getTime(),
                                        e_isRight = isRight,
                                        f_rt = rt*1000)
